...
def how_many(twitter_handles)
  howmany = twitter_handles.count
  return howmany
end
puts how_many(twitter_handles)

#Quel est le handle le plus court de cette liste ?

def the_shortest(twitter_handles)
  shortest = twitter_handles.min{|x,y| x.size <=> y.size}
  return shortest
end
puts the_shortest(twitter_handles)

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)

def five_letters(twitter_handles)
  five = twitter_handles.select {|item| item.size == 6}.size
  return five
end
puts five_letters(twitter_handles)

#Combien commencent par une majuscule (première lettre juste après le @) ?

def upper_first(twitter_handles)
  upper = twitter_handles.count {|handle| handle[1].match?( /[[:upper:]]/ )}
  return upper
end
puts upper_first(twitter_handles)

#Trie la liste de handle par ordre alphabétique.

def alph_order(twitter_handles)
  alphorder = twitter_handles.sort
  return alphorder
end
puts alph_order(twitter_handles)

#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)

def sort_by_length(twitter_handles)
  sort_length = twitter_handles.sort_by(&:length)
  return sort_length
end
puts sort_by_length(twitter_handles)

#Quelle est la position dans l'array de la personne @epenser ?

def epenser_position(twitter_handles)
  epenser_pos = twitter_handles.find_index("@epenser")
  return epenser_pos
end
puts epenser_position(twitter_handles)
